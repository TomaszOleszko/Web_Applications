...
class User
{
    ...
    public function saveDB($db){
        ...
        $values = rtrim($values,", ");
        $db->insert("INSERT INTO users(userName,passwd, fullName,email  , date, status) VALUES ($values)");
    }

    static public function getAllUsersFromDB($db){
        echo $db->select("SELECT * from klienci.users",['userName','fullName','email','passwd','status','date']);
    }
